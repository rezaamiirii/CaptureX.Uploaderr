...
namespace CaptureX.Uploader
{
    public partial class MainForm : Form
    {
                    ...
                    else
                    {
                        ...
                        if (files.Length > 0)
                        {
                            ...
                            foreach (string filePath in files)
                            {
                                ...
                                string sentFullPath = Path.Combine(Properties.Settings.Default.DestinationDirectory, Properties.Settings.Default.SentFilesFolderName, fileName);
        ...
        /// <summary>
        /// this method should calculate the all file count and the remain files and how much done successfully(not complte yet)
        /// </summary>
        ...
        public void CalculateTheCountOfFiles(bool recursively, string sourceFilePath, string destinationFilePath, DateTime fromDate, DateTime toDate)
        {
            var files = Directory.GetFiles(sourceFilePath, "*.dcm").Where(file => File.GetLastWriteTime(file) >= fromDate
                                                                              && File.GetLastWriteTime(file) <= toDate).ToArray();
            int allFiles = files.Length;

            int lblAllFiles = 0;

            if (recursively)
                if (int.TryParse(lblAllValue.Text.Trim(), out lblAllFiles))
                    allFiles += lblAllFiles;
                else
                    allFiles = 0;

            lblAllValue.Text = allFiles.ToString();
            lblRemainValue.Text = allFiles.ToString();

            foreach (var file in files)
            {
                string fileName = Path.GetFileName(file);
                string destinationFile = Path.Combine(destinationPath, fileName);
                string sentFilePath = Path.Combine(destinationPath, Properties.Settings.Default.SentFilesFolderName, fileName);

            }


            foreach (var subDirectory in Directory.GetDirectories(sourceFilePath))
                CalculateTheCountOfFiles(true, subDirectory, destinationFilePath, fromDate, toDate);
        }

        public void ChangeValueOfProgressbar(int copiedFilesCount, int totalFilesToCopy)
        {
        ...
        {
            var files = Directory.GetFiles(sourcePath, "*.dcm").Where(file => File.GetLastWriteTime(file) >= fromDate
                                                                              && File.GetLastWriteTime(file) <= toDate).ToArray();

            int totalFilesToCopy = files.Length;
            int copiedFilesCount = 0;
            int remainFilesCount = totalFilesToCopy;
            int allFiles;
            if (int.TryParse(lblAllValue.Text.Trim(), out allFiles))
                lblAllValue.Text = totalFilesToCopy.ToString();

            lblRemainValue.Text = totalFilesToCopy.ToString();
            ...
            foreach (var file in files)
            {
                ++copiedFilesCount;
                --remainFilesCount;
                lblRemainValue.Text = (totalFilesToCopy - copiedFilesCount).ToString();
                lblDoneValue.Text = (totalFilesToCopy - remainFilesCount).ToString();

                // Report the progress to the provided progress bar
                if (progressBarForFileUpload.Value != 100)
                    ChangeValueOfProgressbar(copiedFilesCount, totalFilesToCopy);
            ...
            try
            {
                using (HttpClient httpClient = new HttpClient())
                {
                    ...
                    else
                    {
                        ...
                        LogError("Invalid User or Password!" + ClinicUsernameTextBox.Text + " & " + ClinicPasswordTextBox.Text);
        ...
        public async Task<DicomClient> SendRequest(string[] files)
        {
            ...
            try
            {
                ...
                foreach (var item in files)
                {
                    ...
                    if (!File.Exists(dicomFile)) throw new FileNotFoundException($"Dicom file in {dicomFile} was not found");
        ...
        private string GetLogFilePath()
        {
            ...
            if (!File.Exists(logFileName))
            {
                using (FileStream fs = File.Create(logFileName))
                {
                    ...
                    string initialContent = "Log file created on " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
